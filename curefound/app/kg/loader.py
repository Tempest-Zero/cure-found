...
# Anchored at the project root (`curefound/`) via `app.core.paths`, not at
# this file's parent -- the loader's on-disk depth changed in the
# pre-Phase-1 refactor (was `kg/loader.py`, now `app/kg/loader.py`).
from app.core.paths import SEED_KG_PATH as DEFAULT_KG_PATH
...
class KG:
    ...
    def subgraph_around(self, node_id: str, k: int = 2, max_nodes: int = 200) -> dict:
        ...
        while queue and len(visited) < max_nodes:
            ...
            neighbors = sorted(set(self.graph.successors(cur)) | set(self.graph.predecessors(cur)))
            for nb in neighbors:
    ...
    def evidence_paths(
        ...
        for node_seq in node_paths:
            ...
            if ok and edge_path:
                scored.append((score, len(node_seq), "->".join(node_seq), edge_path))
...
def _validate(
    ...
    for i, n in enumerate(nodes):
        ...
        if ntype not in KNOWN_NODE_TYPES:
            errors.append(f"node '{nid}': type {ntype!r} not in {sorted(KNOWN_NODE_TYPES)}")
        ...
        for ns, ext in xrefs.items():
            ...
            if prev is not None and prev != nid:
                errors.append(
                    f"xref collision: {ntype} nodes '{prev}' and '{nid}' both claim {ns}='{ext}'"
                )
            else:
                xref_owner[key] = nid
    ...
    for i, e in enumerate(edges):
        ...
        if not (isinstance(h, str) and h and isinstance(r, str) and r and isinstance(t, str) and t):
            errors.append(
        ...
        if r not in KNOWN_RELATIONS:
            errors.append(
                f"edges[{i}] {h}-{r}->{t}: relation {r!r} not in {sorted(KNOWN_RELATIONS)}"
            )
        key = (h, r, t)
        ...
        if r == "TREATS":
            ...
            if not isinstance(yr, int):
                errors.append(f"TREATS edge {h}->{t}: 'approval_year' must be int (got {yr!r})")
...
def load_kg(path: Path | str = DEFAULT_KG_PATH) -> KG:
    ...
    triples = [
        (entity_to_idx[e["head"]], relation_to_idx[e["rel"]], entity_to_idx[e["tail"]])
        for e in edges
    ]
...
if __name__ == "__main__":
    kg = load_kg()
    print(
        f"Loaded KG {kg.version}: {len(kg.idx_to_entity)} entities, "
        f"{len(kg.idx_to_relation)} relations, {len(kg.triples)} triples"
    )
    print(f"  drugs={len(kg.drugs)}  diseases={len(kg.diseases)}  symptoms={len(kg.symptoms)}")
    print(f"  treats_edge: {len(kg.treats_edge)}  xref_index: {len(kg.xref_index)}")
    npc = kg.node("D:NPC")
    if npc:
    ...
    print(f"2-hop subgraph around NPC: {len(sub['nodes'])} nodes, {len(sub['edges'])} edges")
    paths = kg.evidence_paths("DR:AMBROXOL", "D:GAUCHER", k=3, max_paths=5)
    print(f"Ambroxol -> Gaucher evidence paths: {len(paths)}")
    for p in paths[:3]:
        chain = " -> ".join(f"[{x['rel']}:{x['direction']}] {x['to']}" for x in p)

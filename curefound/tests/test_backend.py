...
from kg.loader import KG, KGValidationError, load_kg                    # noqa: E402
...
def test_subgraph_deterministic_across_processes(tmp_path: Path) -> None:
    ...
    assert len(set(results)) == 1, (
        ...
        + "\n".join(f"  seed={s}: {r[:120]}..." for s, r in zip(
            ("1", "2", "3", "7", "42"), results
        ))
    )
...
def test_subgraph_edge_source_target_valid(client: TestClient) -> None:
    ...
    body = client.get(
        "/subgraph", params={"node_id": "D:GAUCHER", "k": 2, "max_nodes": 40}
    ).json()
...
def test_subgraph_respects_max_nodes(client: TestClient) -> None:
    body = client.get(
        "/subgraph", params={"node_id": "D:NPC", "k": 3, "max_nodes": 10}
    ).json()
...
def test_diagnose_accepts_lowercase_hpo(client: TestClient) -> None:
    ...
    body = client.post(
        "/diagnose", json={"symptoms": ["hp:0010729", "hp:0001250"]}
    ).json()
    assert set(body["resolved_inputs"]) == {"S:CHERRYRED", "S:SEIZURES"}
...
def test_repurpose_ranks_within_candidate_set(client: TestClient) -> None:
    body = client.post(
        "/repurpose",
        json={"disease_id": "D:GAUCHER", "top_k": 5,
...
def test_repurpose_include_approved_respected(client: TestClient) -> None:
    body = client.post(
        "/repurpose",
        json={"disease_id": "D:GAUCHER", "top_k": 19,
              "include_already_approved": True},
    ).json()
    cands = body["candidates"]
...
def test_repurpose_accepts_mondo_id(client: TestClient) -> None:
    ...
    body = client.post(
        "/repurpose",
        json={"disease_id": "MONDO:0009937", "top_k": 3,
              "include_already_approved": False},
    ).json()
...
def test_drug_pathway_walker_filters_on_targets(kg: KG) -> None:
    ...
    from api.services.repurpose import RepurposeService
    import numpy as np
...
def test_kg_schema_validation_catches_injected_bugs(tmp_path: Path) -> None:
    ...
    payload["edges"].append(
        {"head": "DR:IMIGLUCERASE", "rel": "TREATS",
         "tail": "D:NOT_REAL", "source": "seed", "approval_year": 2025}
    )
    # 3. TREATS without approval_year.
    payload["edges"].append(
        {"head": "DR:VORINOSTAT", "rel": "TREATS",
         "tail": "D:GAUCHER", "source": "seed"}
    )
...
def test_repurpose_is_deterministic(client: TestClient) -> None:
    payload = {"disease_id": "D:NPC", "top_k": 10,
               "include_already_approved": False}

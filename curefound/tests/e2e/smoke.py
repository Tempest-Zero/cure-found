...
def assert_eq(name, a, b):
    if a != b:
        print(f"FAIL {name}: expected {b}, got {a}")
        sys.exit(1)
    print(f"ok   {name}")
...
def main() -> None:
    # Context manager ensures the FastAPI lifespan (KG load, TransE load) runs.
    with TestClient(app) as c:
        ...
        assert_true(
            "search niem -> NPC hit", any("Niemann-Pick" in n for n in names), f"got {names}"
        )
        ...
        resp = c.post(
            "/repurpose",
            json={"disease_id": "D:NPC", "top_k": 10, "include_already_approved": False},
        ).json()
        assert_true(
            "repurpose NPC has candidates",
            len(resp["candidates"]) >= 5,
            f"got {len(resp['candidates'])} candidates",
        )
        cand_names = [cand["drug_name"] for cand in resp["candidates"]]
        # HP-β-CD or Vorinostat or Rapamycin are repurposing candidates we expect to surface
        known = any(
            "cyclodextrin" in name.lower()
            or "vorinostat" in name.lower()
            or "rapamycin" in name.lower()
            for name in cand_names
        )
        assert_true(
            "repurpose NPC surfaces a known repurposing candidate",
            known,
            f"candidates were {cand_names}",
        )

        # 6. repurpose via MONDO id works
        resp2 = c.post(
            "/repurpose",
            json={"disease_id": "MONDO:0009937", "top_k": 3, "include_already_approved": True},
        ).json()
        ...
        assert_eq(
            "diagnose cherry-red+hypotonia+seizures+devdelay -> Tay-Sachs",
            top,
            "Tay-Sachs disease",
        )
        ...
        assert_eq(
            "diagnose Fabry profile -> Fabry", d["candidates"][0]["disease_name"], "Fabry disease"
        )
        ...
        root = c.get("/")
        assert_true(
            "frontend index loads", root.status_code == 200 and b"CureFound" in root.content
        )
